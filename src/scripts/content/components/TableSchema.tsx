...
function TableSchema(props: { table: DuneTable, onClose: () => void }) {
    const { table } = props;

    return (
        // bounds={{ left: 0, top: 0 }} defaultPosition={{ x: 25, y: 25 }} handle=".windowHeader,contentDiv"
        <Draggable cancel='.react-resizable-handle'>
            <ResizableBox width={200} height={200} minConstraints={[100, 100]} maxConstraints={[600, 600]}>
                <>
